...
from ocp_vscode import show  # Only in dev/preview mode
import math
...
'''
    ...
    def _validate_geometry(self, stl_path: str, constraints: Dict) -> Dict:
        ...
        try:
            ...
            if dims[0] < 100 or dims[0] > 400:
                errors.append(f"Length {dims[0]:.1f}mm outside AFO range [100-400]")
            if dims[2] < 80 or dims[2] > 350:
                errors.append(f"Height {dims[2]:.1f}mm outside AFO range [80-350]")
...
class OctoMCPAgent(BaseAgent):
    ...
    def __init__(self):
        ...
        self._printer_url = "http://localhost:5000"  # OctoPrint default
        self._api_key = ""  # Set via config
    ...
    def _upload_gcode(self, gcode_path: str) -> bool:
        ...
        self._log(f"Upload queued: {gcode_path}")
        return Path(gcode_path).exists()

    def _start_print(self, filename: str) -> bool:
        """Start printing uploaded file."""
        self._log(f"Print start requested: {filename}")
        return True
